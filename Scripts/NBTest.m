...
mdl = load('nb_final.mat').mdl %import saved best model
...
for i = 1:1000 %due to function being quick, looping to improve accuracy of timing
    [predclass, post] = predict(mdl, Xt); %applying the model to the unseen test data
end
...
TestLoss = loss(mdl, Xt, Yt) %calculating the loss on the test data
figure(1)
confusionchart(Yt, predclass) %displaying a confusion chart of the results
e = edge(mdl,Xt,Yt) %calculate test sample edge
m = margin(mdl, Xt, Yt);  %calculate test sample margin

figure
[X,Y,T,AUC] = perfcurve(Yt, post(:,2), 1); %calculate ROC
plot(X,Y) %plot ROC
...
%[~,PosteriorRegion] = predict(mdl,[x1Grid(:),x2Grid(:),x3Grid(:)]); %calculate the posterior probabilities for all of these points
...
%scatter3(Xt{:,1},Xt{:,2},Xt{:,3}, 100, predclass, 'Marker','x','LineWidth', 2) %plot the predicted classes as crosses
...
%view(0,0) %set views [comment out, use (0,0), (90,0) and (90,270)]
